...
class FgPlanner:
    ...
    def __init__(self, robot_scale=0.3302):
    ...
    def plan(self, scan_data, odom_data):
        ...
        # min_speed = self.CORNERS_SPEED
        # max_speed = self.STRAIGHTS_SPEED

        # if np.fabs(steering_angle) > np.pi/8:
        #     speed = min_speed
        # else:
        #     speed = float(-(3/np.pi)*(max_speed - min_speed)*np.fabs(steering_angle)+max_speed)
